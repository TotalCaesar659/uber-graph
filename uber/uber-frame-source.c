...
uber_frame_source_prepare (GSource *source,
                         gint    *timeout_)
{
   UberFrameSource *fsource = (UberFrameSource *)source;
   gint64 current_time;
   guint elapsed_time;
   guint new_frame_num;
   guint frame_time;
...
{
   gint timeout_;
   return uber_frame_source_prepare(source, &timeout_);
}
 ...
 */
guint
uber_frame_source_add (guint       frames_per_sec,
                     GSourceFunc callback,
                     gpointer    user_data)
{
   UberFrameSource *fsource;
   GSource *source;
   guint ret;
   ...
   source = g_source_new(&source_funcs, sizeof(UberFrameSource));
   fsource = (UberFrameSource *)source;
   fsource->fps = frames_per_sec;
   fsource->frame_count = 0;
   fsource->start_time = g_get_monotonic_time() / 1000;
   g_source_set_callback(source, callback, user_data, NULL);
   g_source_set_name(source, "UberFrameSource");
